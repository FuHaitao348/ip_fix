class IPAttnProcessor_mask2_0(nn.Module):
    ...
    def __init__(self, hidden_size, cross_attention_dim=None, rank=4, network_alpha=None, lora_scale=1.0, scale_img=1.0, scale_text=1.0, num_tokens=4,use_mask=True):
        ...
        self.to_k_ip = nn.Linear(cross_attention_dim or hidden_size, hidden_size, bias=False).requires_grad_(False)
        self.to_v_ip = nn.Linear(cross_attention_dim or hidden_size, hidden_size, bias=False).requires_grad_(False)
    ...
    def __call__(
        ...
        if encoder_hidden_states is None:
            ...
            ip_hidden_states_mask = ip_hidden_states
        # for text
        ...
        # TODO: mask for ip_hiddent_states

        if self.use_mask:
            
            ip_key_mask = self.to_k_ip_mask(ip_hidden_states_mask)
            ip_value_mask = self.to_v_ip_mask(ip_hidden_states_mask) 
        ...
        hidden_states = self.scale_text * hidden_states + (self.scale_img * ip_hidden_states) * ip_hidden_states_mask_out #hidden_states + (self.scale * ip_hidden_states)
